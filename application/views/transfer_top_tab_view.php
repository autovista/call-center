...
function test()
{
	...
	$.ajax({
			url:"<?php echo site_url('transfer_lead/telecaller_filter'); ?>",
			type:'POST',
			data:{status:s,fromdate:f,todate:t,transfer_from:b},
			success:function(response){$("#replacediv").html(response);}
			});
                        ...
                        <div class="form-group">
                              <select class="filter_s col-md-12 col-xs-12 form-control" name="user" id="transfer_from" >
									<option value="">Transfer From</option>
										<?php	foreach ($transfer_telecaller as $fetch) {?>
    ...
    <script>
    	  ...
    	  <
			function clear() {
				...
				document.getElementById("transfer_to").innerHTML = '';
