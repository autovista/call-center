...
class add_user_model extends CI_Model {
	...
	public function add_user($fname, $lname, $email, $pnum, $location, $role, $date, $password, $process_id, $group_id) {

		$query = $this -> db -> query("insert into lmsuser(`fname`,`lname`,`email`,`mobileno`,`location`,`role`,`date`,`status`,`is_active`,`password`,`process_id`)values('$fname','$lname','$email','$pnum','$location','$role','$date',1,'Offline','$password','$process_id')");
	...
	public function edit_user($id) {
		$this -> db -> select('u.fname,u.lname,u.mobileno,u.email,u.id,u.role,l.location_id,l.location,p.process_name,p.process_id,g.group_name,g.group_id');
		$this -> db -> from('lmsuser u');
	...
	function update_user($fname, $lname, $email, $pnum, $process_id, $location, $role, $id) {
			
		$query = $this -> db -> query('update lmsuser set fname="' . $fname . '",lname="' . $lname . '",email="' . $email . '",mobileno="' . $pnum . '",process_id="' . $process_id . '",location="' . $location . '",role="' . $role . '"  where id="' . $id . '"');
		
		$this -> db -> select('count(group_id) as count');
		$this -> db -> from('tbl_user_group ug');
		$this -> db -> where('user_id', $id);
		$query1 = $this -> db -> get() -> result();
		$this -> db -> last_query();
		
	echo $group_count = count($this -> input -> post('group_id'));
 ...
 $group_id = $this -> input -> post('group_id');
		...
		if ($query) {
			...
			$this -> session -> set_flashdata('message', '<div class="alert alert-success">  <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>LMS User Updated Successfully...! Please Assign Rights.</strong></div>');
	...
	public function delete_user() {
		...
		if ($query) {
			$this -> session -> set_flashdata('message', '<div class="alert alert-success">  <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>LMS User Deleted Successfully...! </strong>');

		} else {
			$this -> session -> set_flashdata('message', '<div class="alert alert-danger">  <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>LMS User Not Deleted Successfully...!</strong>');
